...
.column[.split-two[
  ...
  .row.bg-black[.content.vmiddle.left[
    ```{r message = FALSE}
    titanic <- read_csv(file = "data/titanic_training.csv", col_names = TRUE)
    ```
  ]]
]]
...
.row.bg-main1[.content.vmiddle.left[
 .font4[But we all know that Jack could have survived]
]]
...
.row.bg-black[.content.vtop.left[
  ...
  titanic.test <- read_csv(file = "data/titanic_test.csv", col_names = TRUE)
  ```
]]
...
.row.bg-black[.content.vtop.left[
  ...
  write.csv(titanic.submit, file = "data/titanic_submission.csv", row.names = FALSE)
  ```
]]
...
.row[.content.center[
<br>
<video contorls muted autoplay width="75%">
  <source src="images/KaggleSubmission.mp4" type="video/mp4">
</video>
]]
